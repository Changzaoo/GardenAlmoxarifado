        ...
        <div className="p-6 max-h-[calc(90vh-200px)] overflow-y-auto">
          {devolucoes.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">Nenhuma devolução registrada</p>
            </div>
          ) : (
            <div className="space-y-3">
              {devolucoes.map((emprestimo, index) => {
                ...
                return (
                  <div key={emprestimo.id || index} className="bg-gradient-to-br from-blue-50 to-blue-100/50 rounded-xl p-4 border border-blue-200 hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="flex items-center gap-2 bg-white rounded-lg px-3 py-2 border border-blue-300">
                        ...
                        <span className="font-bold text-blue-600">+{pontosCalc.total} pts</span>
                      </div>
                    </div>
                        ...
                        {emprestimo.ferramentas?.map((ferramenta, idx) => (
                          <li key={idx} className="text-sm text-gray-700 flex items-center gap-2">
                            ...
                            {ferramenta.quantidade && ferramenta.quantidade > 1 && (
                              <span className="text-xs text-gray-500">x{ferramenta.quantidade}</span>
                            )}
                          </li>
                        ))}
                      </ul>
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <div className="flex gap-4">
                        <span className="text-gray-600">
                          Base: <strong className="text-gray-800">{pontosCalc.base} pts</strong>
                        </span>
                        {pontosCalc.bonus > 0 && (
