...
const AlmoxarifadoJardim = () => {
  ...
  const stats = {
    emprestimosAtivos: Array.isArray(emprestimos) ? emprestimos.filter(e => e.status === 'emprestado').length : 0,
    itensDisponiveis: Array.isArray(inventario) ? inventario.reduce((acc, item) => acc + item.disponivel, 0) : 0,
    itensEmUso: Array.isArray(inventario) ? inventario.reduce((acc, item) => acc + (item.quantidade - item.disponivel), 0) : 0,
    totalItens: Array.isArray(inventario) ? inventario.length : 0
  };
