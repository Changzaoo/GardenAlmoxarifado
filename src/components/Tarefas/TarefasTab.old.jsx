...
const TarefasTab = ({ funcionarios = [] }) => {
  ...
  // Função para verificar se o usuário tem permissão para manipular a tarefa
  const usuarioPodeManipularTarefa = (tarefa) => {
    return (
      tarefa.funcionariosAtribuidos?.some(f => f.id === usuario.id) ||
      tarefa.mencionados?.includes(usuario.id) ||
      tarefa.funcionariosSelecionados?.some(f => f.id === usuario.id) ||
  ...
  const handleIniciarTarefa = async (tarefaId) => {
    try {
      ...
      if (!usuarioPodeManipularTarefa(tarefa)) {
        showToast('Você não tem permissão para iniciar esta tarefa', 'error');
        return;
      }
  ...
  const handleContinuarTarefa = async (tarefaId) => {
    try {
      const tarefa = tarefas.find(t => t.id === tarefaId);
      if (!usuarioPodeManipularTarefa(tarefa)) {
        showToast('Você não tem permissão para continuar esta tarefa', 'error');
        return;
      }
  ...
  const handleConcluirTarefa = async (tarefaId, avaliacao, comentario) => {
    try {
      ...
      // Remove a verificação de permissão para permitir que todos possam concluir tarefas
  ...
  // Filtrar tarefas de acordo com o nível do usuário e filtro selecionado
      ...
      <div className="space-y-8">
        ...
        {tarefasEmAndamentoFiltradas.length > 0 && (
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {tarefasEmAndamentoFiltradas.map((tarefa) => (
                <div
                  key={tarefa.id}
                  onClick={() => setTarefaSelecionada(tarefa)}
                  className="bg-[#192734] p-4 rounded-xl border-l-4 border border-[#38444D] border-l-blue-500 cursor-pointer hover:border-[#1DA1F2] transition-colors"
                ))}
            </div>
          </div>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {tarefasPendentesFiltradas.map((tarefa) => (
                <div
                  key={tarefa.id}
                  onClick={() => setTarefaSelecionada(tarefa)}
                  className="bg-[#192734] p-4 rounded-xl border-l-4 border border-[#38444D] border-l-yellow-500 cursor-pointer hover:border-[#1DA1F2] transition-colors"
                ))}
            </div>
          </div>
        )}
        ...
        {tarefasPausadasFiltradas.length > 0 && (
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {tarefasPausadasFiltradas.map((tarefa) => (
                <div
                  key={tarefa.id}
                  onClick={() => setTarefaSelecionada(tarefa)}
                  className="bg-[#192734] p-4 rounded-xl border-l-4 border border-[#38444D] border-l-orange-500 cursor-pointer hover:border-[#1DA1F2] transition-colors"
                ))}
            </div>
          </div>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {tarefasConcluidasFiltradas.map((tarefa) => (
                <div
                  key={tarefa.id}
                  onClick={() => setTarefaSelecionada(tarefa)}
                  className="bg-[#192734] p-4 rounded-xl border-l-4 border border-[#38444D] border-l-green-500 cursor-pointer hover:border-[#1DA1F2] transition-colors"
          >
